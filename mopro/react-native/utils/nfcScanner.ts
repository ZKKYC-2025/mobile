...
const scanAndroid = async (inputs: Inputs) => {
  return await NfcPassportReader.startReading({
    bacKey: {
      documentNo: inputs.passportNumber,
      expiryDate: inputs.dateOfExpiry,
...
const handleResponseIOS = (response: any) => {
  ...
  const dg2Hash = dg2HashString ? Array.from(Buffer.from(dg2HashString, 'hex')) : undefined;
  ...
  const pem = documentSigningCertificate ? JSON.parse(documentSigningCertificate).PEM.replace(/\n/g, '') : undefined;
